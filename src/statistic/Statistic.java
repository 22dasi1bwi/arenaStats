...
public final class Statistic {
	...
	private Map<Class, Double> calculatePresence(Map<Class, Long> classCounts) {
		...
		return classCounts.entrySet().stream()
				.collect(Collectors.toMap(entry -> entry.getKey(), entry -> new BigDecimal(entry.getValue() / sumOfClassCounts)
				.round(new MathContext(3, RoundingMode.HALF_UP)).doubleValue() * 100));
	}
